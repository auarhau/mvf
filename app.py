...
# ---------- Figur ----------
def build_figure(df_plot: pd.DataFrame, segs, gap_factor, selected_brudd_idx=None):
    ...
    # Highlight valgt brudd
    if selected_brudd_idx is not None and 0 <= selected_brudd_idx < len(segs):
        x0, x1 = segs[selected_brudd_idx]
        fig.add_shape(
        ...
        fig.add_annotation(
            ...
            text=f"Brudd #{selected_brudd_idx + 1}",
    ...
    if uploaded_file:
        try:
            ...
            if COL_DATO not in df_raw.columns:
                st.error(f"Mangler kolonne: '{COL_DATO}'")
                return
            ...
            c3.metric("Datafangst", f"{summary['Datafangst (%)']}%")
            c4.metric("Effektdata", summary["Effektdata tilgjengelig"])

            # Bruddvelger
            selected_brudd_idx = None
            if segs:
                brudd_options = ["Ingen markering"] + [f"Brudd #{item['#']}: {item['Start']} - {item['Slutt']}" for item in brudd_items]
                selected_option = st.selectbox("Fremhev brudd i grafen:", brudd_options)
                if selected_option != "Ingen markering":
                    selected_brudd_idx = int(selected_option.split("#")[1].split(":")[0]) - 1
            ...
            fig = build_figure(df_plot, segs, gap_factor, selected_brudd_idx)
            st.plotly_chart(fig, use_container_width=True)

            # Detaljert tabell
            if brudd_items:
                st.subheader("Bruddlogg")
                st.dataframe(pd.DataFrame(brudd_items), use_container_width=True)
            else:
